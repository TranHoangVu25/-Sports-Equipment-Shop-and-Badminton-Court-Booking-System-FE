...
const BookingTimeline = ({ court, onBack }) => {
  ...
  const getPriceForHalfHour = (timeStr) => {
    ...
    if (matchedRule) {
      return matchedRule.pricePerHour / 2; 
    }
    return 50000; 
  };
  ...
  const handleNavigateToCheckout = () => {
    ...
    sorted.forEach(slot => {
      ...
      if (!currentGroup) {
        currentGroup = {
          ...
          endTimeInMinutes: timeInMinutes + 30,
          pricePerHour: slotPricePerHour,
        ...
        if (
          ...
          currentGroup.pricePerHour === slotPricePerHour
        ) {
          currentGroup.endTimeInMinutes += 30;
          currentGroup.totalPrice += slot.price;
        } else {
    ...
    const finalBookingDetails = groupedBookings.map(g => {
      ...
      const endH = Math.floor(g.endTimeInMinutes / 60);
      const endM = g.endTimeInMinutes % 60;
    ...
    navigate('/court-checkout', {
      state: { 
        court: timelineData || court, 
        selectedDate, 
        selectedSlots, 
        totalAmount, 
        totalHours, 
    ...
    <>
      <div className="fixed inset-0 z-[9999] bg-[#f0f4f8] flex flex-col font-sans overflow-hidden">
        <div className="bg-[#eb5322] text-white px-4 py-4 flex items-center justify-between relative shadow-md z-10">
          ...
          <div onClick={() => setIsDatePickerOpen(true)} className="flex items-center bg-white/20 rounded-sm px-3 py-2 cursor-pointer border border-white/30 hover:bg-white/30 transition-colors">
        ...
        <div className="bg-[#d04316] px-4 py-3 text-[14px] flex items-center gap-6 text-white overflow-x-auto whitespace-nowrap shadow-inner border-t border-white/10 z-10">
          ...
          <div onClick={() => setShowPricingModal(true)} className="flex items-center gap-2 font-bold underline cursor-pointer ml-auto hover:text-yellow-300 transition-colors text-yellow-400">Xem sân & bảng giá</div>
        </div>
        ...
        <div className="relative flex-1 flex flex-col overflow-hidden bg-white">
          ...
          <div ref={timelineRef} className="flex-1 overflow-auto cursor-grab active:cursor-grabbing select-none" onMouseDown={handleMouseDown} onMouseLeave={handleMouseLeave} onMouseUp={handleMouseUp} onMouseMove={handleMouseMove}>
              ...
              {courtsToRender.map((c, index) => (
                <div key={c.courtId} className="flex w-max border-b border-gray-300">
                  ...
                  {timeSlots.slice(0, -1).map((time, idx) => {
                    ...
                    if (status === 'BOOKED') bgColor = 'bg-[#ff4d4f]'; 
                    else if (status === 'LOCKED') bgColor = 'bg-[#9e9e9e]'; 
                    else if (isSelected) bgColor = 'bg-orange-100 border-2 border-[#eb5322] scale-y-105 z-10 shadow-sm'; 

                    return (
                      <div 
                        key={`${c.courtId}-${idx}`} style={{ minWidth: `${cellWidth}px` }} 
          ...
          <div className="absolute right-6 bottom-6 bg-white p-3 rounded-full shadow-xl border border-gray-200 flex items-center gap-3 z-20">
            ...
            <input type="range" min="40" max="120" value={cellWidth} onChange={(e) => setCellWidth(Number(e.target.value))} className="w-32 accent-[#eb5322]"/>
          </div>
        </div>
        ...
        <div className="bg-[#eb5322] text-white px-6 py-5 flex flex-col items-center gap-4 mt-auto shadow-[0_-4px_10px_rgba(0,0,0,0.1)] relative z-[60]">
          <div className="w-full max-w-4xl flex items-center justify-between font-medium">
            <div className="text-[16px] md:text-[18px] font-bold">Tổng giờ: {totalHours}h{totalMins > 0 ? totalMins : '00'}</div>
            <div className="text-[16px] md:text-[18px] font-bold">Tổng tiền: {formatPrice(totalAmount)}</div>
          </div>
          <button 
            disabled={selectedSlots.length === 0}
            onClick={handleNavigateToCheckout}
            className="w-full max-w-4xl bg-[#fbad18] hover:bg-[#e59b12] disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold py-3.5 rounded-md text-[16px] border-none cursor-pointer transition-colors shadow-md uppercase tracking-wider"
          >
            Tiếp theo
          </button>
        </div>
      ...
      {showPricingModal && (
        <div className="fixed inset-0 z-[100000] bg-[#eb5322] flex flex-col font-sans overflow-hidden text-white">
          <div className="px-4 py-4 flex items-center justify-between relative shadow-sm border-b border-white/20">
            ...
            <h1 className="text-[18px] md:text-[22px] font-bold m-0 absolute left-1/2 transform -translate-x-1/2 whitespace-nowrap tracking-wide">Xem sân và bảng giá</h1>
...
const CourtDetailPage = () => {
    ...
    const fetchCourtDetail = async () => {
      ...
      try {
        ...
        if (response.ok && data.status === 200 && data.data) {
          ...
          const coverImg = extractedImages.length > 0 ? extractedImages[0] : "https://images.unsplash.com/photo-1508609349937-5ec4ae374ebf?auto=format&fit=crop&w=1200&q=80";
          let minTime = "05:00"; let maxTime = "23:00";
          ...
          setCourt({
            ...
            rules: "1. Mang giày thể thao đế mềm.\n2. Không hút thuốc trên sân.\n3. Nhận trả sân đúng giờ.",
            pricingRules: result.pricingRules || [], courts: result.courts || []
          });
        } else {
  ...
  const renderPricingTable = () => {
    ...
    Object.keys(dateGroups).forEach(dKey => {
      ...
      Object.keys(daysMap).forEach(day => {
        const rules = daysMap[day].sort((a,b) => a.startTime.localeCompare(b.startTime));
        const signature = rules.map(r => `${r.startTime}-${r.endTime}-${r.pricePerHour}`).join('|');
        if (!signatureMap[signature]) signatureMap[signature] = { days: [], rules: rules };
        signatureMap[signature].days.push(parseInt(day));
      });
      ...
      case 'Thông tin':
        return (
          <div className="space-y-6 animate-fade-in text-gray-700 text-[14px] leading-relaxed">
            <div><h3 className="font-bold text-[16px] text-gray-800 mb-2">Giới thiệu chung</h3><p>{court.description}</p></div>
            <div className="p-4 bg-orange-50 border border-orange-100 rounded-sm">
              <p className="font-medium text-[#eb5322] flex items-center gap-2 mb-2"><Info size={16} /> Lưu ý:</p>
             ...
             <ul className="grid grid-cols-1 sm:grid-cols-2 gap-4">
               {court.services.map((service, index) => (
                 <li key={index} className="flex items-center gap-3 text-[14px] text-gray-700 bg-gray-50 p-3 rounded-sm border border-gray-100">
                   <div className="text-[#eb5322]"><CheckCircleIcon /></div>{service}
                 </li>
               ))}
             </ul>
          </div>
  ...
  if (isLoading) return <div className="w-full min-h-screen flex items-center justify-center bg-gray-50"><Loader2 size={40} className="animate-spin text-[#eb5322]" /></div>;
  if (error || !court) return <div className="w-full min-h-screen flex flex-col items-center justify-center bg-gray-50"><p className="text-gray-600 mb-4">{error || "Sân không tồn tại."}</p><Link to="/courts" className="px-6 py-2 bg-[#eb5322] text-white rounded-sm no-underline font-medium hover:bg-[#d04316] transition-colors">Quay lại hệ thống sân</Link></div>;

  return (
    <>
      {showBookingTimeline && <BookingTimeline court={court} onBack={() => setShowBookingTimeline(false)} />}
      <div className={`bg-gray-100 min-h-screen font-sans pb-20 ${showBookingTimeline ? 'hidden' : 'block'}`}>
        <div className="h-[280px] relative bg-gray-300">
          ...
          <div className="absolute top-0 left-0 w-full p-4 flex justify-between items-center z-10">
            <Link to="/courts" className="bg-white/90 hover:bg-white text-gray-800 rounded-full p-2.5 shadow-sm transition-colors cursor-pointer"><ArrowLeft size={20} strokeWidth={2.5} /></Link>
            <div className="flex gap-3">
              <button className="bg-white/90 hover:bg-white text-gray-800 rounded-full p-2.5 shadow-sm transition-colors border-none cursor-pointer"><Heart size={20} strokeWidth={2.5} className="hover:text-red-500 transition-colors" /></button>
              <button className="bg-white/90 hover:bg-white text-gray-800 rounded-full p-2.5 shadow-sm transition-colors border-none cursor-pointer"><Share2 size={20} strokeWidth={2.5} className="hover:text-blue-500 transition-colors" /></button>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-[1000px] mx-auto px-4 relative z-10 -mt-24 mb-6">
          <div className="bg-white rounded-sm shadow-md p-6 pt-12 relative border border-gray-100">
            ...
            <div className="pr-0 sm:pr-40">
              <h1 className="text-[20px] md:text-[22px] font-bold text-gray-800 mt-2 mb-2 leading-snug">{court.name}</h1>
              <div className="inline-flex items-center px-2.5 py-1 bg-blue-50 border border-blue-100 text-blue-600 rounded-sm text-[12px] font-medium mb-5">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4 mr-1.5"><path d="M12 22v-5"></path><path d="M9 7l3 10 3-10"></path><path d="M14 2c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2z"></path><path d="M10 2c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2z"></path><path d="M6 7l6 10"></path><path d="M18 7l-6 10"></path></svg>{court.category}
              </div>
              <div className="space-y-3.5 text-[14px] text-gray-600 font-medium">
                <div className="flex items-start gap-3"><MapPin className="text-[#eb5322] mt-0.5 flex-shrink-0" size={18} strokeWidth={2.5} /><span className="leading-snug">{court.address}</span></div>
                <div className="flex items-start gap-3"><Clock className="text-[#eb5322] mt-0.5 flex-shrink-0" size={18} strokeWidth={2.5} /><span>{court.time}</span></div>
                <div className="flex items-start gap-3"><Phone className="text-[#eb5322] mt-0.5 flex-shrink-0" size={18} strokeWidth={2.5} /><div className="flex flex-col"><a href={`tel:${court.phone}`} className="text-[#eb5322] font-semibold hover:underline no-underline block">{court.phone}</a></div></div>
              </div>
              <div className="mt-6 block sm:hidden">
                <button onClick={() => setShowBookingTimeline(true)} className="w-full bg-[#f0ad4e] hover:bg-[#e09d3e] text-white font-bold py-3 rounded-sm transition-colors shadow-sm text-[14px] border-none cursor-pointer">ĐẶT LỊCH NGAY</button>
              </div>
            </div>
            <div className="flex overflow-x-auto border-b border-gray-200 mt-8 scrollbar-thin scrollbar-thumb-gray-300 w-full pb-0.5">
              {TABS.map(tab => (
                <button key={tab} onClick={() => setActiveTab(tab)} className={`whitespace-nowrap py-3.5 px-6 font-bold text-[14px] transition-colors bg-transparent cursor-pointer border-t-0 border-l-0 border-r-0 border-b-[3px] outline-none ${activeTab === tab ? 'border-[#eb5322] text-[#eb5322]' : 'border-transparent text-gray-500 hover:text-gray-800 hover:bg-gray-50/50'}`}>{tab}</button>
              ))}
            </div>
          </div>
